...
function buildPanchangShape(p, ctx) {
    ...
    const tithiName = tithiNameRaw ? t(`tithi.${toKey(tithiNameRaw)}`, lang) : tithiNameRaw;
    const nakName = nakNameRaw ? t(`nakshatra.${toKey(nakNameRaw)}`, lang) : nakNameRaw;
    const yogName = yogNameRaw ? t(`yog.${toKey(yogNameRaw)}`, lang) : yogNameRaw;
    const karanName = karanNameRaw ? t(`karan.${toKey(karanNameRaw)}`, lang) : karanNameRaw;

    const sunSign = sunSignRaw ? t(`rashi.${toKey(sunSignRaw)}`, lang) : sunSignRaw;
    const moonSign = moonSignRaw ? t(`rashi.${toKey(moonSignRaw)}`, lang) : moonSignRaw;

    const pakshaNorm = normalizePaksha(pakshaRaw); // Krishna Paksha / Krishna-Paksha -> Krishna-Paksha
    const paksha = pakshaNorm ? t(`paksha.${toKey(pakshaNorm)}`, lang) : pakshaRaw;

    return {
        day: p?.day ? t(`day.${toKey(p.day)}`, lang) : getLocalDayName(ctx?.datetime, tz, lang),
...
module.exports = function getPanchang(payload) {
    return new Promise((resolve, reject) => {
        ...
        execFile(pythonBin, [script, JSON.stringify(payload)], (err, stdout, stderr) => {
            ...
            if (raw.debug) response.debug = raw.debug;
            if (raw.offset_sweep) response.offset_sweep = raw.offset_sweep;
            if (raw.lagna_chart_ui) {
                response.lagna_chart = lang === "hi" ? localizeLagnaChart(raw.lagna_chart_ui, lang) : raw.lagna_chart_ui;
            }
            if (raw.planets_detailed) {
                response.planets = (lang === "hi") ? localizePlanets(raw.planets_detailed, lang) : raw.planets_detailed;
            }
            if (raw.mhadasha) {
                response.mhadasha = (lang === "hi") ? localizeMhadasha(raw.mhadasha, lang) : raw.mhadasha;
            }
            if (raw.navamsa_chart) {
                response.navamsa_chart = (lang === "hi") ? localizeLagnaChart(raw.navamsa_chart, lang) : raw.navamsa_chart;
            }
            if (raw.divisional_charts) {
                response.divisional_charts = (lang === "hi") ? localizeDivisionalCharts(raw.divisional_charts, lang) : raw.divisional_charts;
            }
            if (raw.Vershful) {
                response.Vershful = (lang === "hi") ? localizeVershful(raw.Vershful, lang) : raw.Vershful;
            }
