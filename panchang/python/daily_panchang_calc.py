...
from durmuhurta import compute_durmuhurta
from varjyam import compute_varjyam
from amrit_kaal import compute_amrit_kaal
from godhuli import compute_godhuli_muhurta
from nishita_kaal import compute_nishita_kaal
...
def get_next_karana(tithi_no: int, half_tithi: int) -> str:

    current_n = (tithi_no - 1) * 2 + half_tithi + 1
    next_n = current_n + 1
    if next_n == 58:
        return "Shakuni"
    if next_n == 59:
...
def daily_calc_panchang(dt_local, dt_utc, lat: float, lon: float, ref_jd: float, mode):
    ...
    next_tithi_no = tithi_no + 1
    if next_tithi_no > 30:
        next_tithi_no = 1

    next_tithi_name = get_tithi_name(next_tithi_no)

    nak_no, nak_name, charan, nak_offset = compute_nak_charan(moon)
    next_nak_no = nak_no + 1
    if next_nak_no > 27:
        next_nak_no = 1
    ...
    next_yoga_no = yoga_no + 1
    if next_yoga_no > 27:
        next_yoga_no = 1

    next_yoga_name = YOGAS[next_yoga_no - 1]

    half_tithi = int((diff % 12) / 6)  # 0 or 1
    karana_name = get_karana(tithi_no, half_tithi)
    next_karana_name = get_next_karana(tithi_no, half_tithi)
    ...
    # Hindu Maah Details
    hindu_maah = compute_hindu_maah(daily_jd)
    
    date_local = datetime.now(tz) 
    vikram = get_vikram_samvat(date_local)
    shaka = get_shaka_samvat(date_local, ref_jd)

    durmuhurta = []
    varjyam = []
    amrit_kaal = []
    godhuli_muhurta = None
    nishita_kaal = None
    sunrise_local_new = jd_ut_to_dt_utc(sunrise_jd).astimezone(tz)
    sunset_local_new = jd_ut_to_dt_utc(sunset_jd).astimezone(tz)

    if sunset_local_new <= sunrise_local_new:
        sunset_local_new = sunset_local_new + timedelta(days=1)

    durmuhurta = compute_durmuhurta(sunrise_local, sunset_local_new)

    nak_end_dt = None
    if ends["nak_ms"]:
        nak_end_dt = datetime.fromtimestamp(ends["nak_ms"] / 1000, tz)

    # Approx nakshatra start = previous boundary
    # Current nakshatra duration approx from current time to nak end
    # Better exact start later karenge, abhi V1 ke liye dt_local use kar rahe hain
    if nak_end_dt:
        varjyam = compute_varjyam(nak_no, dt_local, nak_end_dt)

    if nak_end_dt:
        amrit_kaal = compute_amrit_kaal(nak_no, dt_local, nak_end_dt)

    godhuli_muhurta = compute_godhuli_muhurta(sunset_local)
    nishita_kaal = compute_nishita_kaal(sunset_local)
    ...
    panchangObj = {
        ...
        "tithi": {
            "details": {
                ...
                "next_tithi_number": next_tithi_no,
        ...
        "yog": {
            "details": {
                "yog_number": yoga_no,
                "yog_name": yoga_name,
                "next_yog_number": next_yoga_no,
                "next_yog_name": next_yoga_name,
                "special": None,
                "meaning": None
            },
        ...
        "karan": {
            "details": {
                ...
                "next_karana_name": next_karana_name,
                "special": None,
                "deity": None
            },
