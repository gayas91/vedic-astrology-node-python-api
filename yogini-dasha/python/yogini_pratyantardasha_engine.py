...
def main():
    try:
        ...
        if mahadasha_input not in YOGINI_NAME_TO_INDEX:
            raise ValueError(
                f"Invalid mahadasha value: {raw_mahadasha}. Use one of MAN, PIN, DHA, BHR, BHA, ULK, SID, SAN."
            )

        if antardasha_input not in YOGINI_NAME_TO_INDEX:
            raise ValueError(
                f"Invalid antardasha value: {raw_antardasha}. Use one of MAN, PIN, DHA, BHR, BHA, ULK, SID, SAN."
            )
        ...
        result = {
            "title": f"{selected_maha['yogini']}-{selected_antar['_key']}",
